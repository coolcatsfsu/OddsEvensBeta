...
public class MainActivity extends AppCompatActivity implements View.OnTouchListener, CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    private Mat mP1;
    private Mat mP2;
    private int numFingersP1 = 0;
    private int numFingersP2 = 0;
    long mP1addr = 0;
    long mP2addr = 0;
    Intent pIntent;
    int pConfig;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pIntent = getIntent();
        pConfig = pIntent.getIntExtra("player_assign",0);
        ...
        fade_badge = AnimationUtils.loadAnimation(getBaseContext(),R.anim.abc_shrink_fade_out_from_bottom);
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        this.fingerNumber = this.fingerNumber - 2;
        if (this.fingerNumber > 2) {
            this.fingerNumber = 2;
        }
        else if(this.fingerNumber < 1)
        {
            this.fingerNumber = 1;
        }
    ...
    public void showBadge(View v, int badgeNumber) {
        ...
        //badge.startAnimation(fade_badge);
        badge.setVisibility(View.INVISIBLE);
    }
    ...
    public void takePlayer1pic(View v)
    {
        mP1 = new Mat();
        mP1 = rgba.clone();
        Core.flip(mP1.t(), mP1, 180);
        mP1addr = mP1.getNativeObjAddr();
        numFingersP1 = fingerNumber;
        showBadge(v, 1);
        switchUser(v);
    }
    public void takePlayer2pic(View v)
    {
        mP2 = new Mat();
        mP2 = rgba.clone();
        Core.flip(mP2.t(), mP2, 180);
        mP2addr = mP2.getNativeObjAddr();
        numFingersP2 = fingerNumber;
        showBadge(v, 2);
    }
    public void showWinner(View v) {
        int total = numFingersP1 + numFingersP2;
        if ( ((total % 2) == 0) && pConfig == 0 )
        {
            winner = 0;
        }
        else
        ...
        */
        ...
        intent.putExtra("mP1addr", mP1addr);
        intent.putExtra("mP2addr", mP2addr);
